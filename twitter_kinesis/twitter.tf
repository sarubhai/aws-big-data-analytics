...
data "aws_ami" "twitter_kinesis_linux" {
...
# User Data Init
data "template_file" "twitter_kinesis_init_script" {
  template = templatefile("${path.module}/twitter_server.sh", {
    ...
    kinesis_stream_name    = var.twitter_kinesis_stream_name,
  })
}


# EC2 Instance
resource "aws_instance" "twitter_kinesis_server" {
  ami                    = data.aws_ami.twitter_kinesis_linux.id
  ...
  tags = {
    Name  = "TwitterKinesisServer"
    Owner = var.owner
  }

  user_data = data.template_file.twitter_kinesis_init_script.rendered
}
